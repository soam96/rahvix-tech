...
export default async function ServiceDetail({
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-100">
      ...
      {/* Hero Section */}
      <section className="relative pt-36 pb-20 px-6 overflow-hidden border-b border-slate-900 bg-slate-50/40">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-7">
            <Link 
              href="/#services" 
              className="inline-flex items-center gap-2 text-xs font-mono text-slate-500 hover:text-slate-900 transition-colors mb-8 uppercase tracking-wider font-bold"
            >
            ...
            <div className="flex items-center gap-3 mb-5">
              <span className={`px-3 py-1 text-[10px] font-mono font-bold tracking-widest rounded bg-white/80 border border-slate-200/85 uppercase ${foundCategory.colorClass}`}>
                {foundCategory.title}
              </span>
              <span className="text-[10px] font-mono text-slate-500 border border-slate-200 px-2.5 py-1 rounded bg-white/80 uppercase font-semibold">
                {foundService.id}
              </span>
            </div>
            ...
            <a 
              href="/#contact"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-slate-900 font-extrabold transition-all hover:scale-105 active:scale-95 shadow-lg"
          ...
          <div className="lg:col-span-5 flex justify-center">
            <div className="w-full max-w-[400px] aspect-[4/3] rounded-2xl border border-slate-200 bg-white/30 p-3 shadow-2xl relative overflow-hidden backdrop-blur-md">
              ...
              <div className="absolute top-5 left-5 px-2.5 py-0.5 rounded bg-slate-50/80 border border-slate-200 font-mono text-[8px] font-bold text-slate-500 tracking-widest uppercase">
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {foundService.benefits.map((benefit, i) => (
                <div key={i} className="flex items-start gap-3 p-4 rounded-xl border border-slate-850 bg-white/20 backdrop-blur-sm">
          ...
          <div>
            ...
            <ul className="flex flex-col gap-3">
              {foundService.deliverables.map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-slate-600 border-b border-slate-900 pb-3 last:border-0">
        ...
        <div className="lg:col-span-5">
          <div className="sticky top-32 flex flex-col gap-6">
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Indian Market Pricing Models</h2>
            
            {foundService.pricing.map((tier, i) => (
              <div 
                key={i} 
                className="relative creative-card p-6 rounded-2xl flex flex-col bg-white/40 border border-slate-850 backdrop-blur-sm"
                style={{ borderTopWidth: i === 1 ? "4px" : "1px", borderTopColor: i === 1 ? foundCategory.accentColor : "" }}
              >
                {i === 1 && (
                  <span 
                    className="absolute -top-3 right-6 px-3 py-0.5 rounded-full text-[10px] font-bold text-slate-900 uppercase tracking-wider shadow-md"
                ...
                <p className="text-slate-500 text-sm mt-1 mb-4 min-h-[40px] font-medium">{tier.description}</p>
