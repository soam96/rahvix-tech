    ...
    <section id="about" className="relative py-24 px-6 overflow-hidden bg-transparent">
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          >
            <div className="creative-card p-8 rounded-2xl bg-slate-900/30 border border-slate-850 relative overflow-hidden backdrop-blur-sm">
              <div className="absolute top-0 right-0 w-24 h-24 bg-brand-orange/5 rounded-bl-[100px] border-b border-l border-slate-850" />
              <h3 className="text-xl font-extrabold text-white mb-4">Our Core Philosophy</h3>
              ...
              <div className="mt-6 flex flex-col gap-4 font-mono text-[10px] text-slate-450 uppercase font-semibold">
            ...
            {pillars.map((pillar, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="creative-card p-6 rounded-2xl bg-slate-900/20 border border-slate-850/80 hover:border-slate-700/80 transition-colors duration-300 relative group flex flex-col justify-between cursor-default"
                >
                  <div className="flex flex-col gap-4">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center border border-slate-800 bg-slate-950 shadow-sm shrink-0" style={{ boxShadow: `0 4px 10px ${pillar.bgGlow}` }}>
                      <PillarIcon className={`w-4 h-4 ${pillar.colorClass}`} />
                    </div>
                    <div>
                      <h4 className="text-md font-bold text-white mb-2">{pillar.title}</h4>
                      <p className="text-slate-350 text-xs leading-relaxed font-medium">{pillar.desc}</p>
                    </div>
                  </div>
                </motion.div>
              );
